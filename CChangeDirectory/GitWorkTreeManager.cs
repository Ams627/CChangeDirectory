...
namespace CChangeDirectory
{
    public class GitWorkTreeManager
    {
        ...
        private readonly List<GitWorktreeInfo> _worktreeList = new List<GitWorktreeInfo>();

        private const string HeadPattern = @"ref:\s+refs/heads/([^\s]+)\s*$";
        public GitWorkTreeManager(string dir)
        {
            _dir = dir;
        ...
        /// </summary>
        public GitWorktreeInfo[] WorkTrees => _worktreeList.ToArray();
        ...
        private List<GitWorktreeInfo> GetGitSubDirs(string dir, bool isSecondaryWorktree)
        {
            ...
            result.Add(new GitWorktreeInfo {
                Directory = mainDirInfo.FullName,
                BranchName = GetGitBranchName(Path.Combine(mainDirInfo.FullName, ".git", "HEAD")),
                Description = GetDescription(mainDirInfo.FullName)
            });
            ...
            if (Directory.Exists(workTreesDir))
            {
                ...
                foreach (var (gitdir, head) in gitFiles)
                {
                    ...
                    result.Add(new GitWorktreeInfo {
                        Directory = parentDir,
                        BranchName = GetGitBranchName(head),
                        Description = GetDescription(parentDir)
                    });
                }
            }
        ...
        {
            var descriptionFilename = Path.Combine(dir, ".description");
            if (!File.Exists(descriptionFilename))
            {
                return "";
            }
            return File.ReadAllLines(descriptionFilename).First();
        }
